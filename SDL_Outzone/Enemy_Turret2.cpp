...
#define ENEMY_SHOOT_SPEED 3
#define ENEMYSHOOTDELAY 1750
		...
		//if (currentTime > (lastTime + 0)) {
			if (left == true){
				if ((angle < 90) && (angle >= 0)){
					...
					App->particles->AddParticle(App->particles->enemyshoot, original_x + 0, original_y + 43, COLLIDER_ENEMY_SHOT);
				}
			}
			else{
				if ((angle < 90) && (angle >= 0)){
					App->particles->enemyshoot.speed.x = ENEMY_SHOOT_SPEED * sin(angle * ANGLE_CONVERT_REVERSE);
					App->particles->enemyshoot.speed.y = ENEMY_SHOOT_SPEED * cos(angle * ANGLE_CONVERT_REVERSE);
					App->particles->AddParticle(App->particles->enemyshoot, original_x + 36, original_y + 43, COLLIDER_ENEMY_SHOT);
				}
			}
